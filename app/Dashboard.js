...
const Dashboard = () => {
  ...
  const { data: todayAvail, refetch } = useQuery({
    queryKey: ["workerTodayAvailabilityFetch2"],
    queryFn: () => workerTodayAvailabilityFetch(),
  });
  const actionsBtns = [
    {
      id: 1,
      name: todayAvail?.availabilityStatus != "leave" ? "Active" : "Leave",
      iconName: "user",
      iconFrom: "Feather",
    },
  ...
  const submitLeaveRequest = async () => {
    const res = await workerRequestLeave({
      startDate: fromDate,
      endDate: toDate,
    });
    if (!res?.error) {
  ...
  return (
    <>
                  ...
                  return (
                    ...
                    >
                      <Text
                        style={{
                          ...styles.actionText,
                          color: item?.name == "Leave" ? "red" : "#026874",
                        }}
                      >
                        {" "}
                        {item.name}{" "}
      ...
      <View>
        <CommonModal
          state={"cancelApnt"}
          cancelApntPopup={cancelModal}
          setterFunc={setCancelModal}
        />
        <CommonModal
          state={"activeToday"}
          cancelApntPopup={activeTodayPopup}
          onCloseClick={() => {
            refetch();
            setActiveTodayPopup(false);
          }}
          setterFunc={setActiveTodayPopup}
        />
        <CommonModal
          state={"leaveReqSent"}
          cancelApntPopup={leaveReqSent}
          setterFunc={setLeaveReqSent}
        />
                ...
                <View style={styles.btnsFlexBx}>
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      router.navigate("BuyWaste"), setBuyWaste(false);
                    }}
                    style={styles.buywasteBtn}
                  >
                    <Text style={styles.buywasteBtnText}>Buy Waste</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.buywasteBtn}>
                    <Text style={styles.buywasteBtnText}>
                      View Customers History
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
          ...
          <View style={styles.buyWasteBx}>
            ...
            <View style={styles.dateinptFlex}>
              <View style={styles.inptWrap}>
                ...
                <TouchableOpacity
                  onPress={() => handleCalendarFunc("FromDate")}
                  activeOpacity={0.87}
                  style={styles.inputBx}
                >
                  <TextInput
                    style={{ fontSize: 13, color: "#828181" }}
                    keyboardType="numeric"
                    value={fromDate}
              ...
              <View style={styles.inptWrap}>
                ...
                <TouchableOpacity
                  onPress={() => handleCalendarFunc("ToDate")}
                  activeOpacity={0.87}
                  style={styles.inputBx}
                >
                  <TextInput
